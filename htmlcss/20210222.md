...
- 그렇다면 새로운 표준 HTML5는 무엇이 다를까?
  1. `새로운 컨텐츠 모델 등장`: 이전에는 기능에 따라 block인지 inline인지만 구분하고 inline은 block보다 무조건 작기 때문에 block 태그 안에 포함될 수만 있었다. (e.g. block 태그인 <div> 안에 <span>, vice versa는 불가능)
    ...
    - e.g. `<a>`는 inline element이기 때문에 원래는 다른 block 요소를 포함할 수 없었으나, 이제는 `<h1>`, `<p>` 등을 포함할 수 있다. 하지만 interactive영역이기 때문에 같은 interactive 영역이라는 속성을 가진 `<button>` 등은 포함할 수 없다.
...
### 디자인 시안 분석 방법
- 보통은 psd 파일을 받아 분석하거나, 요즘은 figma라는 UI developing tool로 만들어진 파일을 받기도 한다.
...
#### html 틀 짜기
- header와 main contenst, footer 부분을 파악한다. (3단으로 나누면 되겠구나~)
  - 어떤 구조가 적합하고, 어떤 태그를 쓸 것인지 결정할 때는 그 이유를 설명할 수 있어야 한다. 
  - `div`로 다 때려박던 것은 빈약한 명령어를 가졌던 HTML4.0시절의 일이고, 이제는 HTML5로 인해 풍성해진 semantic tag들을 잘 활용해야 한다. (사람도, 기계도 알아들을 수 있는 기능적 태그들인 header, footer, nav 등이 있는데 왜 쓰질 못하니!)
...
```
- html 기본 틀 짜기의 핵심은 **(1) 디자인 시안의 논리적 흐름을 파악하고, (2) semantic mark-up을 완성한 후 (3) naming까지 하는 것** 
...
- `<title>`: 탭에 나타나는 부분. 
  - `SEO(Search Engine Optimization)`을 잘 준수하기만 하면 검색 시 가중치가 잘 붙어 적은 노력과 비용으로도 홍보효과가 향상될 수 있으니  괜히 특수문자나 환영인사 같은 거 넣지 말고 관련 단어를 넣는다. 
  - 우리는 '웹카페-HTML5, CSS3, 웹표준, 웹접근성'라고 넣자.
- `<link rel="stylesheet" href="css/style.css">`를 **추가**해서 'style.css'라는 파일을 만들어 거기에 우리의 꾸미기 코드를 쌓자. 
...
- main 파트에 3개의 열이 있는데, 이를 어떻게 처리할지 고민해보자.
  ...
  - div 3개로 grouping하여 처리하기 
    - Emmet Abbreviation: `div.group.group${그룹$}*3`을 하면 위와 같이 각 숫자를 넣은 div 3개가 등장한다. 
여기까지 하고 이제 css로 이들을 꾸며주자.

#### CSS 코드 짜기
- CSS는 구조와 표현의 분리, one source multiuse를 위한 것임을 명심 (요즘은 component 단위로 만들기 때문에 또 다르다는 이야기도...)
...
- CSS의 기본 룰은 선택하고, 선택한 공간에 대해 선언해주는 것이다. 선언은 `:`으로, 여러 속성은 `;`로 구분해준다.
  ...
  - selector는 클래스의 이름이며 property는 굉장히 다양하다. 많이 알면 알수록 많은 스타일을 구사할 수 있다. 
- CSS 배치를 위한 속성과 값
  - `position`, `float`, `display: flex`, `display: grid` 등의 방법이 있으며, 앞 2개는  클래식한 방법, flex는 조금 더 모던하고 직관적이기 때문에 쉽다. 그러나 IE11 외의 익스플로러에서는 호환이 잘 안된다는 단점이 있다.
...
- CSS로 layout을 코딩할 때 가급적 높이는 세팅하지 않는 것이 좋다. 새로운 contents가 추가될 때마다 조정해야하는 문제.
  - 굳이 한다면 절대 추가되지 않을 부분(header 정도)만 지정해주기.
  - 여기서는 visual 영역에 높이값을 줄 것 (광고마다 높이크기가 달라지면 안되므로)
  - 나머지는 기본값 그대로 auto로 두되, 최소 높이 default를 `min_height=400`를 주어 컨텐츠가 없거나 적어도 유지되게끔 한다.
  - 위에는 이미 해주었지만 css 파일을 link 해준다. `<link rel="stylesheet" href="css/style.css">`
...
- float
  - `display: float; float: left;`로 띄워서 좌측으로 쌓아줄 수 있다.
  - 어떤 요소를 float하면 그 요소의 성격은 content의 width를 따라가는 inline-block으로 변한다.
  - group 1~3을 float처리하니까 그 다음 slogan이 위로 올라왔다. 이를 벗어나기 위해서는 slogan에 `clear: both;`를 해줘야 한다. 
    - 하지만 다음 요소를 건드리는 건 좋은 방법이 아님
    - 또한 slogan에 윗여백을 줘도 이미 float으로 확보된 영역이 margin으로 처리되어 여백이 먹히지 않음.
  ...
  - 다음 시간에는 float하면서 잃어버린 자식요소의 높이를 부모요소가 어떻게 하면 읽어들일 수 있는지 알아보자.
