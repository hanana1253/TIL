...
```
- 만약 모듈파일이 아니라면 xhr.js 전체 코드를 감싸면서 위 객체를 리턴하는 ajax 함수를 만들고 ajax.get 등으로 사용하면 된다. 
- state.js에서는 `import { get, post, patch, remove } from './utils/xhr.js';` 그대로 사용하면 된다.
...
- Promise 객체는 settled 된 경우 resolve/reject의 인자로 온 값을 들고 있으며, 이를 쓰려면 후속처리 메서드를 사용한다.
  ...
  - 모든 then에서 발생하는 에러는 마지막 catch에서 잡을 수 있으므로 가독성 및 편리한 일괄 처리를 위해 마지막 catch 한 번으로 에러 처리 해준다.
...
export default {
  ...
  // ... post, patch, delete에서도 callback 빼고  생략
}
```
