...
- 현재 만들어둔 getRandom.test.js와 transformText.test.js를 찾아 실행하려다가 실패한다.
  - 우리는 import로 getRandom.js와 transformText.js의 함수들을 가져왔는데, 바벨은 번들러가 아니라서 nodejs로 테스트할 시 commonJS로 파일을 만든다. 
...
- 이제 getRandom.test.js와 transformText.test.js에서 우리가 만든 tests.js를 import하지말고, jest로 테스트해보면 터미널에서 테스트 결과를 볼 수 있다.