...
- `$ npx eslint --init`으로 아래와 같이 기본설정을 한다.
  - To check syntax and find problems / Javascript modules(import/export) / React / No(Typescript) / all(Browser and node.js) / JSON (json파일로 config 관리) / YES(plugin도 설치) 
- 이제 package.json 파일에 eslint와 eslint-plugin-react가 깔려있고 eslintrc.json 파일도 생성되어있다.
- eslintrc.json 파일에는 parser 옵션에 jsx가 true로 설정되어있는데 이는 React용으로 lint를 했기 때문
  ...
  - 이렇게 run 하면 react version이 not specified라고 뜨는데 이는 윌가 아직 React를 안 깔았기 때문이다. eslintrc.json에서 "plug-in"에 있는 "plugin:react/recommended"와 "extends"에 있는 react를 주석처리해준다.
